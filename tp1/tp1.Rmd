...
# df <- select(cafe_sales_df, "Item", "Quantity", "Payment Method", "Location")
# head(df, 20)
```
...
cafe_sales_df <- cafe_sales_df %>%
  mutate(`Price Per Unit` = case_when(  # aplico condiciones
    Item %in% c(NA, "ERROR", "UNKNOWN") ~ NA_real_,  # Si el Item es NA, ERROR o UNKNOWN, deja NA
    `Price Per Unit` %in% c(NA, "ERROR", "UNKNOWN") ~ menu_df$Price_Per_Unit[match(Item, menu_df$Item)], # Si el precio es NA, ERROR o UNKNOWN, asigna el del menú
